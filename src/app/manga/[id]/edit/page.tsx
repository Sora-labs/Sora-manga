...
const EditMangaPage = () => {
  ...
  const formProps = useForm({
    resolver: zodResolver(updateMangaSchema),
  });
  const {
    control,
    handleSubmit,
  ...
  const handleConvertPreviewItem = useCallback(() => {
    ...
    const convertData: MangaDetail = {
      ...
      coverImage:
        typeof previewData.coverImage === "string"
          ? previewData.coverImage
          : convertFileObjectToImage(previewData.coverImage),
      backgroundImage:
        typeof previewData.backgroundImage === "string"
          ? previewData.backgroundImage
          : convertFileObjectToImage(previewData.backgroundImage),
      // use preview
  ...
  useEffect(() => {
    (async () => {
      ...
      if (response.data?.is_success) {
        ...
        const selectedTags = detail.tags.map((tag: any) => tag._id);
        const previewTags = detail.tags.map((tag: any) => ({
          ...tag,
          label: tag.name,
        }));
        setValue("tags", selectedTags);
        setValue("previewTags", previewTags);
        setValue("coverImage", detail.cover_url);
        setValue("backgroundImage", detail.background_url);
  ...
  return (
    <div className="flex mt-12 py-2 gap-4">
      <div className="px-3 w-full md:w-1/2 lg:w-2/5">
        <h1 className="my-2 text-2xl">Edit {getValues().name}</h1>
